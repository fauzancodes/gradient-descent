...
#gradient descent function, using loss function = mean square error
def gradient_descent(x, y, slope, intercept, learning_rate, minimum_gradient, maximum_iterations) :
    ...
    while looping == "continue" :
        ...
        gradient_slope = np.mean(-2 * x * (y - ((slope * x) + intercept)))
        gradient_intercept = np.mean(-2 * (y - ((slope * x) + intercept)))
