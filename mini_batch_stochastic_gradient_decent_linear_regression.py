...
def gradient_descent(x, y, initial_a, initial_b, learning_rate, mini_batch) :
    ...
    while len(x_random) <= mini_batch :
        ...
        x_random.append(x[random_scalar])
        y_random.append(y[random_scalar])
    
    x_random = np.array(x_random)
    y_random = np.array(y_random)

    gradient_a = np.sum(-2 * x_random * (y_random - ((initial_a * x_random) + initial_b)))
    gradient_b = np.sum(-2 * (y_random - ((initial_a * x_random) + initial_b)))
...
while looping == "continue" :
    initial_a, gradient_a, initial_b, gradient_b, ssr  = gradient_descent(x, y, initial_a, initial_b, learning_rate, mini_batch)
