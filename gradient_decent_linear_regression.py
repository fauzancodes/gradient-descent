...
def gradient_descent(x, y, initial_a, initial_b, learning_rate) :
    ...
    gradient_a = np.sum(-2 * x * (y - ((initial_a * x) + initial_b)))
    gradient_b = np.sum(-2 * (y - ((initial_a * x) + initial_b)))
...
plt.subplot(2, 2, 2)
plt.scatter(np.arange(0, len(tot_a)), tot_a, label = "The Slope", c = "red")
plt.scatter(np.arange(0, len(tot_b)), tot_b, label = "The Intercept", c = "blue")
plt.legend(loc = "lower right")
plt.xlabel("Number of Iterations", fontdict = font2)
plt.ylabel("Amplitude", fontdict = font2)
plt.title(("(Number of Iteration, Amplitude) " + ", alpha: " + str(learning_rate)), fontdict = font1)
